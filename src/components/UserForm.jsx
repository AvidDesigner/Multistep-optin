...
export default class UserForm extends Component {
    ...
    validateAndNext = async (isApp) => {
        this.formSubmitted = true
        await this.validate().then(error => {
            this.next(error, isApp)
        })
    }

    next = (error, isApp) => {
        if(error.name === '' && error.email === '' && error.phone === '') {            
            isApp ? this.setState({step: 10}) : this.setState({step: this.state.step + 1})
        }
    }
    ...
    handleWebsiteFormSubmit = () => {
        ...
        axios.post('https://optin-emailer.herokuapp.com/send', {
            name: user.name,
            messageHtml: message,
            senderEmail: 'noreply@xaplemedia.com', //<-- Email 
            receiverEmail: 'info@s4c.app', //<-- Receiver Email Address Here,
            SMTPHost: 'leader.herosite.pro',
            SMTPPort: 465,
            SMTPUser: 'noreply@xaplemedia.com',
            SMTPPassword: '4HU=ecxB(ex5'
        });
    }
    ...
    multiStepForm = () => {
        const { step, user, isApp } = this.state;
        ...
        switch (step) {
            ...
            case 3:
                return (
                    <UserDetailsForm user={user} changeUserData={this.changeUserData} prevStep={this.prevStep} isApp={isApp} nextStep={this.nextStep} errors={this.state.errors} validateForm={this.validateAndNext}/>
                )
